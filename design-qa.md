...
- “Données & évolutions” departmental synthesis and methodology dialogs: present, wired and tested from the sidebar action;
...
6. P1: the departmental synthesis still referenced the removed point-grid dataset (`state.data.heat`) and failed on opening. Fix: it now reads the 28,105 morphoclimatic polygons, uses the shared “Données & évolutions” naming from the other thematic maps and opens a complete departmental synthesis. Post-fix evidence: `implementation-synthesis.png` and a clean console.
